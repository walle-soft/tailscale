...
import (
	...
	linux_abi "gvisor.dev/gvisor/pkg/abi/linux"
...
var (
	iptablesChainNames = map[int]string{
		linux_abi.NF_INET_PRE_ROUTING:  "PREROUTING",
		linux_abi.NF_INET_LOCAL_IN:     "INPUT",
		linux_abi.NF_INET_FORWARD:      "FORWARD",
		linux_abi.NF_INET_LOCAL_OUT:    "OUTPUT",
		linux_abi.NF_INET_POST_ROUTING: "POSTROUTING",
	}
...
func enumerateIptablesTable(logf logger.Logf, table string, cb func(string, *entry) error) error {
	...
	var tableName linux_abi.TableName
	copy(tableName[:], []byte(table))

	tbl := linux_abi.IPTGetinfo{
		Name: tableName,
	}
	slt := sockLen(linux_abi.SizeOfIPTGetinfo)
	...
	entsBuf := make([]byte, linux_abi.SizeOfIPTGetEntries+tbl.Size)
	entsHdr := (*linux_abi.IPTGetEntries)(unsafe.Pointer(&entsBuf[0]))
	...
	// Skip header
	entsBuf = entsBuf[linux_abi.SizeOfIPTGetEntries:]
...
func (p *entryParser) parseEntry(b []byte) (entry, error) {
	...
	ret.NFCache = endian.Native.Uint32(p.getBytes(4))
	targetOffset := int(endian.Native.Uint16(p.getBytes(2)))
	nextOffset := int(endian.Native.Uint16(p.getBytes(2)))
	/* Comeback = */ p.getBytes(4)
	ret.PacketCount = endian.Native.Uint64(p.getBytes(8))
	ret.ByteCount = endian.Native.Uint64(p.getBytes(8))
...
func (p *entryParser) parseIPTIP() (iptip, error) {
	...
	const IFNAMSIZ = 16
	ret.InputInterface = btos(p.getBytes(IFNAMSIZ))
	ret.OutputInterface = btos(p.getBytes(IFNAMSIZ))

	ret.InputInterfaceMask = p.getBytes(IFNAMSIZ)
	ret.OutputInterfaceMask = p.getBytes(IFNAMSIZ)

	ret.Protocol = endian.Native.Uint16(p.getBytes(2))
	ret.Flags = p.getByte()
...
func (p *entryParser) parseMatches(maxOffset int) ([]match, error) {
	...
	for {
		...
		matchSize := int(endian.Native.Uint16(p.getBytes(2)))
		curr.Name = btos(p.getBytes(XT_EXTENSION_MAXNAMELEN))
			...
			if len(curr.RawData) >= 12 {
				curr.Data = matchTCP{
					SourcePortRange: [...]uint16{
						endian.Native.Uint16(curr.RawData[0:2]),
						endian.Native.Uint16(curr.RawData[2:4]),
					},
					DestPortRange: [...]uint16{
						endian.Native.Uint16(curr.RawData[4:6]),
						endian.Native.Uint16(curr.RawData[6:8]),
					},
...
func (p *entryParser) parseTarget(nextOffset int) (target, error) {
	...
	targetSize := int(endian.Native.Uint16(p.getBytes(2)))
	ret.Name = btos(p.getBytes(XT_EXTENSION_MAXNAMELEN))
	...
	case "standard":
		if len(ret.RawData) >= 4 {
			verdict := int32(endian.Native.Uint32(ret.RawData))
	...
	case "ERROR":
		ret.Data = errorTarget{
			ErrorName: btos(ret.RawData),
		}

	case "REJECT":
		if len(ret.RawData) >= 4 {
			ret.Data = rejectTarget{
				With: rejectWith(endian.Native.Uint32(ret.RawData)),
			}
		}

	case "MARK":
		if len(ret.RawData) >= 8 {
			mark := endian.Native.Uint32(ret.RawData[0:4])
			mask := endian.Native.Uint32(ret.RawData[4:8])
